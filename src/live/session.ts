...
import { backendInstructions, LIVE_INSTRUCTIONS, LIVE_TOOLS } from "./vault-tools";
...
export class VaultLiveSession {
  ...
  async start(apiKey: string, backendModel: string, context: string): Promise<void> {
    ...
    try {
      if (!apiKey.trim()) throw new Error("Add your OpenAI API key in FJG File Focus settings.");
  ...
  private onEvent(event: Record<string, any>): void {
    ...
    if (event.type === "session.started") {
      ...
      void this.loop.handle(event).catch(() => this.fail("Could not finish a voice note request. Check the dashboard before trying the change again."));
    }
  }
