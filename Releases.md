...
- Rehydrated hidden File Focus leaves that Obsidian restores as generic deferred views, preventing a blank native sidebar tab even when **Open on Start** is disabled.
- Retried deferred-leaf hydration across the mobile layout-restoration window and when native sidebar selection or layout changes occur.
...
- Recreated stale restored leaves that have not finished binding to the updated plugin instead of leaving an unusable blank tab.
